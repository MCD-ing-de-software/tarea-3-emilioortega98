...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        df_for_trim = df.copy()
        df_for_trim["name"] = df_for_trim["name"].astype("string")

        result = cleaner.trim_strings(df_for_trim, ["name"])

        self.assertEqual(df_for_trim.loc[0, "name"], " Alice ")
        self.assertEqual(result.loc[0, "name"], "Alice")
        self.assertEqual(result.loc[1, "name"], "Bob")
        self.assertEqual(result.loc[3, "name"], "Carol")

        pdt.assert_series_equal(result["city"], df_for_trim["city"], check_names=True)
